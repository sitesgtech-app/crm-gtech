...
export const Layout: React.FC<LayoutProps> = ({ children, user, onLogout, onUserUpdate }) => {
  ...
  useEffect(() => {
      ...
      if (user) {
          const fetchNotifs = () => setNotifications(db.getNotifications(user.id));
          fetchNotifs();
  ...
  useEffect(() => {
      ...
      // THEME COLORS MAP (PRESETS)
      const themes: Record<string, any> = {
          'blue': { primary: '#3b82f6', sidebar: '#0f172a' },
          'green': { primary: '#10b981', sidebar: '#064e3b' },
          'purple': { primary: '#8b5cf6', sidebar: '#2e1065' },
          'red': { primary: '#ef4444', sidebar: '#450a0a' },
          'dark': { primary: '#6366f1', sidebar: '#000000' }
      };

      // Prioritize User Theme Selection if available, else Org Colors
      const activeTheme = (user && user.theme) ? themes[user.theme] : null;
      
      if (activeTheme) {
          root.style.setProperty('--brand-500', activeTheme.primary);
          root.style.setProperty('--brand-600', activeTheme.primary);
          ...
          root.style.setProperty('--brand-700', activeTheme.primary);
          root.style.setProperty('--brand-800', activeTheme.primary);
          root.style.setProperty('--brand-900', activeTheme.primary);
      } else if (org?.brandColors) {
          const { primary, secondary, sidebar } = org.brandColors;
          ...
          root.style.setProperty('--brand-800', primary); 
          root.style.setProperty('--brand-900', primary); 
      } else {
          // Fallback
          ...
          {filteredMenuItems.map((item, idx) => {
                    ...
                    <div key={idx} className="mb-2">
                        <button
                            ...
                            className={`w-full flex items-center justify-between px-3 py-2.5 text-sm font-medium rounded-lg transition-all duration-200 group ${
                                isActiveParent ? sidebarTextClass : `${sidebarTextSecondaryClass} hover:${sidebarTextClass}`
                            }`}
                        >
                                ...
                                {item.children.map((child, cIdx) => {
                                    ...
                                    return (
                                        <Link
                                            ...
                                            className={`flex items-center px-4 py-2 text-xs font-medium rounded-r-lg transition-all relative ${
                                                isChildActive 
            ...
            return (
              <Link
                ...
                className={`flex items-center px-3 py-2.5 mb-1 text-sm font-medium rounded-lg transition-all duration-200 ${
                  isActive 
                    ? sidebarActiveClass 
