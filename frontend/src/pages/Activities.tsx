    ...
    const activityReport = useMemo(() => {
        ...
        return users.map(u => {
            ...
            return {
                ...
                recent: userActs.slice(0, 3), // Last 3 activities (default view)
                allActs: userActs, // All activities for detailed view
                proposalCount,
                proposalAmount
            };
        ...
        <div className="space-y-6">
            <div className="flex flex-col md:flex-row justify-between md:items-center gap-4">
                ...
                <div className="flex items-center gap-4">
                    {/* Slider Control for Detailed Logs */}
                    <div className="flex items-center mr-4">
                        <label className="flex items-center cursor-pointer relative">
                            <input
                                ...
                                onChange={() => setShowDetailedLogs(!showDetailedLogs)}
                            />
                            <div className="w-11 h-6 bg-slate-200 peer-focus:outline-none peer-focus:ring-4 peer-focus:ring-brand-300 rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-brand-600"></div>
                            <span className="ml-3 text-sm font-medium text-slate-700">Vista Detallada</span>
                        </label>
                    </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                {activityReport.map((data) => {
                    // Determine which activities to show and how
                    // If showDetailedLogs is TRUE: Reverse order (newest first), All items (or larger set), Timestamps included, Full text
                    // If showDetailedLogs is FALSE: Default order (oldest first as per original slice), Only 3 items, No time, Truncated text
                    const activitiesToShow = showDetailedLogs
                        ? [...data.allActs].reverse() // Reverse chronological
                        : data.recent;
                            ...
                            <div className="p-4">
                                ...
                                <div>
                                    <h4 className="text-xs font-bold text-slate-700 mb-3 uppercase flex items-center gap-1">
                                        <Calendar className="w-3 h-3" /> {showDetailedLogs ? 'Historial Completo' : 'Últimas Gestiones'}
                                    </h4>
                                    <div className={`space-y-3 ${showDetailedLogs ? 'max-h-96 overflow-y-auto pr-2' : ''}`}>
                                        {activitiesToShow.length > 0 ? (
                                            activitiesToShow.map(act => (
                                                <div key={act.id} className="flex gap-3 items-start text-sm border-b border-slate-50 pb-2 last:border-0">
                                                    <div className={`w-1.5 h-1.5 mt-1.5 rounded-full shrink-0 ${act.type === 'Llamada' ? 'bg-brand-500' :
                                                    ...
                                                    <div className="w-full">
                                                        ...
                                                        <p className={`text-xs text-slate-500 ${showDetailedLogs ? '' : 'line-clamp-1'}`}>
                                                            {act.description}
                                                        </p>
                                                    </div>
                                                </div>
