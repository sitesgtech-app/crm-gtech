...
export const SalesGoals: React.FC<SalesGoalsProps> = ({ user }) => {
    ...
    useEffect(() => {
        // Load users from API (Real Backend)
        const fetchUsers = async () => {
            try {
                const response = await api.get('/users');
                if (response.data && Array.isArray(response.data)) {
                    // Filter only Sellers or Admins
                    const team = response.data.filter((u: User) => u.role === UserRole.VENDEDOR || u.role === UserRole.ADMIN);
                    setUsers(team);
                } else {
                    // Fallback to local
                ...
                console.error("Error fetching users for Goals:", error);
                const allUsers = db.getUsers().filter(u => u.role === UserRole.VENDEDOR || u.role === UserRole.ADMIN);
                setUsers(allUsers);
            }
        };
                                ...
                                <div className="p-5 space-y-6">
                                    ...
                                    <div>
                                        ...
                                        <div className="h-2.5 w-full bg-slate-100 rounded-full overflow-hidden">
                                            <div
                                                className={`h-full rounded-full transition-all duration-1000 ${stats.revenuePercent >= 100 ? 'bg-emerald-500' :
