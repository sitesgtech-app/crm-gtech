                  ...
                  >
                    ...
                    <div className="flex justify-between items-start">
                      ...
                      <span className={`text-[9px] font-bold px-1.5 py-0.5 rounded uppercase flex items-center gap-1 ${
                          opp.sector === 'Gubernamental' ? 'bg-amber-50 text-amber-700 border border-amber-100' : 'bg-slate-50 text-slate-500 border border-slate-100'
                      }`}>
                          {opp.sector || 'Privado'}
                      </span>
                    </div>
                      ...
                      <div className="flex gap-2 mb-3 flex-wrap items-center">
                         <div className="relative inline-block">
                            <select 
                                ...
                                className={`text-xs font-bold px-3 py-1.5 rounded-full uppercase appearance-none pr-8 cursor-pointer outline-none border transition-all ${
                                    selectedOpp.stage === OpportunityStage.GANADA ? 'bg-green-100 text-green-700 border-green-200' : 
                          ...
                          <div className="flex gap-2 mt-2 overflow-x-auto pb-1">
                              {['Llamada', 'Correo', 'WhatsApp', 'Reunión', 'Visita Técnica'].map(type => (
                                  <button 
                                    ...
                                    className={`px-3 py-1 text-[10px] font-bold rounded-full border transition-colors whitespace-nowrap ${
                      ...
                      <div className="space-y-0 relative pl-4 border-l-2 border-slate-100 ml-2">
                          {currentActivities.map((activity) => (
                              <div key={activity.id} className="mb-6 relative pl-6 group">
                                  <div className={`absolute -left-[25px] top-0 w-8 h-8 rounded-full flex items-center justify-center border-4 border-white shadow-sm ${
                                      activity.type === 'Llamada' ? 'bg-blue-100 text-blue-600' :
