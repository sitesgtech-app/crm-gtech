...
export const updateClient = async (req: Request, res: Response) => {
    try {
        ...
        const cleanData = Object.entries(data).reduce((acc, [key, value]) => {
            ...
            if (['id', 'createdAt', 'updatedAt', 'organizationId'].includes(key)) {
                return acc;
            }

            if (typeof value === 'string' && value.trim() === '') {
                // Protect required fields from becoming null
                if (key === 'name') {
                    acc[key] = data[key]; // Keep original string (even if empty, strictly) or don't set
                } else {
                    acc[key] = null;
                }
            } else if (typeof value === 'string') {
